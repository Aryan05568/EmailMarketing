...
<table cellpadding="0" cellspacing="0" border="0" style="margin: 20px 0;">
    ...
    const startSending = async () => {
        try {
            ...
            // formData.append('userId', userProfile?.id || ''); // Pass user ID if available
            ...
            if (result.success) {
                // updateUserSentCount(userProfile.id, sentCount);
    ...
    //             // Update user's sent count in database when completed
    //             if (userProfile?.id && result.sent > 0) {
    //                 try {
    //                     await updateUserSentCount(userProfile.id, result.sent);
    //                     // console.log(`Updated sent count for user ${userProfile.id}: ${status.sent}`);
    //                 } catch (error) {
    //                     console.error('Failed to update user sent count:', error);
    //                     // You might want to show a notification to user about this error
    //                 }
    //             }
    //         }
    //         }
    ...
    //     }
    // };
    const lastUpdatedSentCountRef = useRef(null);
    ...
    const checkSendingStatus = async (jobId) => {
        try {
            ...
            // Add logs for newly sent emails
            if (sent > sentCount) {
                setSendingLogs(prev => [
                    {
                        id: Date.now(),
                        time: new Date().toLocaleTimeString(),
                        message: `Successfully sent ${sent - sentCount} more emails`,
                        type: 'success'
                    },
                    ...prev.slice(0, 49)
                ]);
            ...
            // Add logs for newly failed emails
            if (failed > failedCount) {
                setSendingLogs(prev => [
                    {
                        id: Date.now() + 1,
                        time: new Date().toLocaleTimeString(),
                        message: `${failed - failedCount} emails failed to send`,
                        type: 'error'
                    },
                    ...prev.slice(0, 49)
                ]);
            ...
            if (result.completed || result.stopped) {
                ...
                // Update sent count in DB only once
                if (
                    userProfile?.id &&
                    sent > 0 &&
                    lastUpdatedSentCountRef.current !== sent
                ) {
                    try {
                        await updateUserSentCount(userProfile.id, sent);
                        lastUpdatedSentCountRef.current = sent;
                    } catch (error) {
                        console.error('Failed to update user sent count:', error);
                    }
    ...
    const updateUserSentCount = async (userId, sentCount) => {
        try {
            const response = await fetch(`${BASEURL}/api/auth/users/${userId}/update-sent-count`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!response.ok) {
                throw new Error(result.message || 'Failed to update sent count');
            }

            console.log('Sent count updated successfully:', result);
            return result;
        } catch (error) {
                ...
                <div className={`${activeStep === 2 ? 'block' : 'hidden'}`}>
                    <div className="bg-white rounded-lg shadow-md p-8 mb-8">
                        ...
                        <div className="mb-4">
                            ...
                            <div className="mb-4">
                                <label htmlFor="email-user" className="block mb-1 font-medium">Sender Email</label>
                                <input
                                    type="email"
                                    id="email-user"
                                    className="w-full p-2 border border-gray-300 rounded-md"
